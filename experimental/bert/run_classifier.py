...
@tf_utils.locate_device(FLAGS.device)
  ...
  def model_fn(features, labels, mode, params):  # pylint: disable=unused-argument
    ...
    if mode == tf.estimator.ModeKeys.TRAIN:
      ...
      train_op = optimization.create_optimizer(
          total_loss, learning_rate, num_train_steps, num_warmup_steps, hvd)
...
def main(_):
  ...
  master_process = True
  training_hooks = []
  num_cluster, _ = common_utils.get_cluster_num(FLAGS.device)
  global_batch_size = FLAGS.train_batch_size * num_cluster
  hvd_rank = 0

  config = tf.compat.v1.ConfigProto(log_device_placement=True, allow_soft_placement=True)
  def _device_fn(op):
      if op.type == 'IteratorV2':
          return '/cpu:0'
  ...
  if FLAGS.horovod:
     ...
     hvd_rank = hvd.rank()
     if FLAGS.device.lower() in ['gpu', 'xla_gpu']:
       config.gpu_options.visible_device_list = str(hvd.local_rank())
     elif FLAGS.device.lower() in ['dtu', 'xla_dtu']:
       config.dtu_options.visible_device_list = str(hvd.local_rank())
     if hvd.size() > 1:
  ...
  training_hooks.append(estimator_utils.LogTrainRunHook(global_batch_size, logger, hvd_rank, FLAGS.save_checkpoints_steps, ignore_steps_num=20, fetches=['global_step_assign:0']))
  ...
  if FLAGS.do_train:
    ...
    if FLAGS.horovod:
      train_file = [os.path.join(FLAGS.output_dir, "train.tf_record{}".format(i)) for i in range(hvd.size())]
  ...
  if FLAGS.do_train:
    file_based_convert_examples_to_features(
        train_examples[start_index:end_index], label_list, FLAGS.max_seq_length, tokenizer, train_file[hvd_rank])
    ...
    train_time_elapsed = training_hooks[-1].all_time
    ...
    if master_process:
        logger.info("-----------------------------")
        logger.info("Total Training Time: {} for Sentences = {}".format(train_time_elapsed,
                num_train_steps * global_batch_size))
        logger.info("Total Training Time W/O Overhead: {} for Sentences = {}".format(train_time_wo_overhead,
                (num_train_steps - training_hooks[-1].skipped) * global_batch_size))
        logger.info("Throughput Average (sentences/sec) with overhead = {}".format(avg_sentences_per_second))
        logger.info("Throughput Average (sentences/sec) = {}".format(ss_sentences_per_second))
        logger.info("-----------------------------")
  ...
  if FLAGS.do_eval and master_process:
    ...
    eval_hooks = [estimator_utils.LogEvalRunHook(FLAGS.eval_batch_size)]
    result = estimator.evaluate(input_fn=eval_input_fn, steps=eval_steps, hooks=eval_hooks)

    estimator_utils.summary_inference_statistics(eval_hooks[-1], FLAGS.eval_batch_size, logger)
