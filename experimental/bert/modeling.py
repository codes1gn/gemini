...
def dropout(input_tensor, dropout_prob):
    ...
    if ((dropout_prob is None) or (dropout_prob == 0.0)):
        return input_tensor
    output = gemini_plugin.dropout(input_tensor, (1.0 - dropout_prob))
    return output

def layer_norm(input_tensor, name=None):
    'Run layer normalization on the last dimension of the tensor.'
    input_tensor = MonadicTensor(input_tensor)
    return gemini_plugin.layer_norm(inputs=input_tensor, begin_norm_axis=(-1), begin_params_axis=(-1), scope=name)
...
def embedding_lookup(input_ids, vocab_size, embedding_size=128, initializer_range=0.02, word_embedding_name='word_embeddings', use_one_hot_embeddings=False):
    ...
    if use_one_hot_embeddings:
        ...
        output = gemini_plugin.matmul(one_hot_input_ids, embedding_tables)
    else:
        output = gemini_plugin.gather(embedding_tables, flat_input_ids)
    input_shape = get_shape_list(input_ids)
    output = gemini_plugin.reshape(output, (input_shape[0:(-1)] + [(input_shape[(-1)] * embedding_size)]))
    output = gemini_plugin.all_gather(output)
...
def embedding_postprocessor(input_tensor, use_token_type=False, token_type_ids=None, token_type_vocab_size=16, token_type_embedding_name='token_type_embeddings', use_position_embeddings=True, position_embedding_name='position_embeddings', initializer_range=0.02, max_position_embeddings=512, dropout_prob=0.1):
    ...
    if use_token_type:
        ...
        token_type_embeddings = gemini_plugin.matmul(one_hot_ids, token_type_table)
        token_type_embeddings = gemini_plugin.reshape(token_type_embeddings, [batch_size, seq_length, width])
    ...
    if use_position_embeddings:
        ...
        with tf.control_dependencies([assert_op]):
            ...
            position_embeddings = gemini_plugin.reshape(position_embeddings, position_broadcast_shape)
...
def attention_layer(from_tensor, to_tensor, attention_mask=None, num_attention_heads=1, size_per_head=512, query_act=None, key_act=None, value_act=None, attention_probs_dropout_prob=0.0, initializer_range=0.02, do_return_2d_tensor=False, batch_size=None, from_seq_length=None, to_seq_length=None):
    ...
    def transpose_for_scores(input_tensor, batch_size, num_attention_heads, seq_length, width):
        output_tensor = gemini_plugin.reshape(input_tensor, [batch_size, seq_length, num_attention_heads, width])
        output_tensor = gemini_plugin.transpose(output_tensor, [0, 2, 1, 3])
        return output_tensor
    query_layer = gemini_plugin.dense(from_tensor_2d, (num_attention_heads * size_per_head), activation=query_act, name='query', kernel_initializer=create_initializer(initializer_range))
    key_layer = gemini_plugin.dense(to_tensor_2d, (num_attention_heads * size_per_head), activation=key_act, name='key', kernel_initializer=create_initializer(initializer_range))
    value_layer = gemini_plugin.dense(to_tensor_2d, (num_attention_heads * size_per_head), activation=value_act, name='value', kernel_initializer=create_initializer(initializer_range))
    ...
    attention_scores = gemini_plugin.matmul(query_layer, key_layer, transpose_b=True)
    attention_scores = gemini_plugin.multiply(attention_scores, (1.0 / math.sqrt(float(size_per_head))))
    if (attention_mask is not None):
        attention_mask = tf.expand_dims(attention_mask, axis=[1])
        adder = ((1.0 - tf.cast(attention_mask, tf.float32)) * (-10000.0))
        attention_scores = MonadicTensor(attention_scores)
        attention_scores += adder
    attention_probs = gemini_plugin.softmax(attention_scores)
    attention_probs = dropout(attention_probs, attention_probs_dropout_prob)
    value_layer = gemini_plugin.reshape(value_layer, [batch_size, to_seq_length, num_attention_heads, size_per_head])
    value_layer = gemini_plugin.transpose(value_layer, [0, 2, 1, 3])
    context_layer = gemini_plugin.matmul(attention_probs, value_layer)
    context_layer = gemini_plugin.transpose(context_layer, [0, 2, 1, 3])
    if do_return_2d_tensor:
        context_layer = gemini_plugin.reshape(context_layer, [(batch_size * from_seq_length), (num_attention_heads * size_per_head)])
    else:
        context_layer = gemini_plugin.reshape(context_layer, [batch_size, from_seq_length, (num_attention_heads * size_per_head)])
...
def reshape_to_matrix(input_tensor):
    ...
    output_tensor = gemini_plugin.reshape(input_tensor, [first_dim, width])
    return output_tensor
